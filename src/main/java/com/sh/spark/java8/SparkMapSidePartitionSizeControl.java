...
public class SparkMapSidePartitionSizeControl {
	...
	public static void controlMapSidePartitionSize(String inputPath, String outputPath, int noOfPartitions,
			long batchSize, long minPartitionSize) throws Exception {
		...
		if (minPartitionSize > 0) {
			sc.hadoopConfiguration().setLong("mapreduce.input.fileinputformat.split.minsize", minPartitionSize);
		}
	...
	public static long computePartitionSize(long goalSize, long minPartitionSize, long blockSize) {
		...
		System.out.println(partitionSize + "=Math.max(minSize=" + minPartitionSize + ", Math.min(goalSize=" + goalSize
				+ ", blockSize=" + blockSize + "))");
		return partitionSize;
	}

	public static void printSizeOfEachPartition(String inputPath, long partitionSize, boolean splitOverall)
			throws IOException {
	...
	public static void main(String[] args) throws Exception {
		...
		int scenarioIndex = 1;
		/*
		 * Scenario 1=Default NoOfPartions = Default Block Size In Local Mode = 32MB
		...
		// SparkMapSidePartitionSizeControl.controlMapSidePartitionSize(inputPath,
		// outputPathPrefix+scenarioIndex,noOfPartitions,blockSize,minPartitionSize);
		scenarioIndex++;

		/*
		 * Scenario 2=Control Number of Partitions. NoOfPartions = 30 Block Size In
		 * Local Mode = 32 MB minPartitionSize=1 byte(Defined in Hadoop code)
		 */
		noOfPartitions = 30;
		blockSize = 0;// default
		...
		SparkMapSidePartitionSizeControl.controlMapSidePartitionSize(inputPath, baseOutputPath + scenarioIndex,
				noOfPartitions, blockSize, minPartitionSize);
		scenarioIndex++;

		/*
		 * Scenario 3=Increase Partition Size(The Wrong Way) NoOfPartions = 5 Block Size
		 * In Local Mode = 32MB minPartitionSize=1 byte(Defined in Hadoop code)
		 */
		noOfPartitions = 5;
		blockSize = 0;// Default
		minPartitionSize = 0;// Default
		...
		/*
		 * Scenario 4=Increase Partition Size(The Right Way) NoOfPartions = 0 Block Size
		 * In Local Mode = 32MB minPartitionSize= 64MB
		 */
